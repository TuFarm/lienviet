...
<div data-xf-init="' . ($__vars['checkStrength'] ? 'password-strength' : null) . ' ' . ($__vars['hideShow'] ? 'password-hide-show' : '') . '"
	data-show-text="' . $__templater->filter('Hiện', array(array('for_attr', array()),), true) . '" data-hide-text="' . $__templater->filter('Ẩn', array(array('for_attr', array()),), true) . '">
	';
	if ($__vars['hideShow']) {
		$__finalCompiled .= '
		...
		<script class="js-zxcvbnLanguage" type="application/json">
			{
				...
				"This is a top-10 common password": "' . $__templater->filter('Mật khẩu này xuất hiện trong danh sách 10 mật khẩu phổ biến nhất.', array(array('escape', array('json', )),), true) . '",
				"This is a top-100 common password": "' . $__templater->filter('Mật khẩu này xuất hiện trong danh sách 100 mật khẩu phổ biến nhất.', array(array('escape', array('json', )),), true) . '",
				"This is a very common password": "' . $__templater->filter('Mật khẩu này rất phổ biến.', array(array('escape', array('json', )),), true) . '",
				"This is similar to a commonly used password": "' . $__templater->filter('Mật khẩu này tương tự như một mật khẩu đã được sử dụng.', array(array('escape', array('json', )),), true) . '",
				...
				"The chosen password could be stronger": "' . $__templater->filter('Mật khẩu được chọn có thể mạnh hơn nữa.', array(array('escape', array('json', )),), true) . '",
				"Entering a password is required": "' . $__templater->filter('Bắt buộc nhập mật khẩu.', array(array('escape', array('json', )),), true) . '",
				"This is a very strong password": "' . $__templater->filter('Đây là một mật khẩu rất mạnh.', array(array('escape', array('json', )),), true) . '",
				"This is a reasonably strong password": "' . $__templater->filter('Đây là một mật khẩu khá mạnh.', array(array('escape', array('json', )),), true) . '"
			}
		</script>
	';
	}
