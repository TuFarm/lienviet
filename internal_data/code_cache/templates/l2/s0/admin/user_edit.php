...
<div class="block">
	...
	';
		...
		if ($__vars['user']['is_admin']) {
			$__compilerTemp6 .= '
					<li><a href="' . $__templater->func('link', array('admins/edit', $__vars['user'], ), true) . '">' . ($__vars['user']['is_super_admin'] ? 'Super Administrator' : 'Quản trị viên') . '</a></li>
				';
		}
		$__compilerTemp6 .= '
				';
	...
	if ($__templater->method($__vars['user'], 'exists', array())) {
		...
		array(
			...
			'hint' => 'Email xác nhận đặt lại mật khẩu sẽ được gửi cho người dùng và họ sẽ không thể đăng nhập cho đến khi họ đặt mật khẩu mới.',
			'_type' => 'option',
		),
		array(
			'value' => 'change',
			'label' => 'Thiết lập mật khẩu mới' . $__vars['xf']['language']['label_separator'],
	...
	if ($__vars['user']['user_id']) {
		...
		$__compilerTemp13 .= $__templater->formRow('
			...
			'data-xf-click' => 'overlay',
							...
							<div>
								' . $__templater->button('Sửa ảnh', array(
			'href' => $__templater->func('link', array('users/avatar', $__vars['user'], ), false),
			'data-xf-click' => 'overlay',
			'class' => 'button--link',
		), '', array(
	...
	array(
		'value' => 'email_confirm',
		'label' => 'Đợi xác nhận thư điện tử',
		'_type' => 'option',
	),
	array(
	...
	array(
		...
		'label' => 'Giới thiệu',
		'hint' => 'Bạn có thể sử dụng BB code',
	)) . '
				</div>
	...
	array(
		...
		'label' => 'Theo dõi nội dung vừa tạo',
	)) . '
	...
	array(
		...
		'label' => 'Theo dõi nội dung vừa tương tác',
	)) . '
