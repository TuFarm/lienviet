...
';
	...
	if (!$__templater->test($__vars['prefixesGrouped'], 'empty', array())) {
		...
		$__compilerTemp5 .= '
				' . $__templater->formRow('
					' . $__templater->filter('Không có', array(array('parens', array()),), true) . ' <a href="' . $__templater->func('link', array('thread-prefixes/add', ), true) . '" target="_blank">' . 'Add prefix' . '</a>
				', array(
			'label' => 'Available Prefixes',
		)) . '
	...
	array(
		...
		'label' => 'Include threads from this forum when users click "What\'s New?"',
		'hint' => 'If disabled, threads from this forum will never appear in the list of new / unread threads when users run /find-new/threads.',
		'_type' => 'option',
	)), array(
