<?php
// FROM HASH: 4667ae5438d9eccf2836c138c3264a41
return array(
...
'status_sticky' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['thread']['sticky']) {
		$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--sticky" aria-hidden="true" title="' . $__templater->filter('Dán lên cao', array(array('for_attr', array()),), true) . '"></i>
...
'statuses' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['showWatched'] AND $__vars['xf']['visitor']['user_id']) {
		...
		if ($__vars['thread']['Watch'][$__vars['xf']['visitor']['user_id']]) {
			$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--watched" aria-hidden="true" title="' . $__templater->filter('Chủ đề đang theo dõi', array(array('for_attr', array()),), true) . '"></i>
								<span class="u-srOnly">' . 'Chủ đề đang theo dõi' . '</span>
							</li>
							';
...
'main_cell' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
			<div class="structItem-title" uix-href="' . $__templater->func('link', array('threads' . (($__templater->method($__vars['thread'], 'isUnread', array()) AND (!$__vars['forceRead'])) ? '/unread' : ''), $__vars['thread'], ), true) . '">
				';
...
'meta_cell' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		$__finalCompiled .= '
		<div class="structItem-cell structItem-cell--meta" title="' . $__templater->filter('Điểm phản ứng tin nhắn đầu tiên' . $__vars['xf']['language']['label_separator'], array(array('for_attr', array()),), true) . ' ' . $__templater->filter($__vars['thread']['first_post_reaction_score'], array(array('number', array()),), true) . '">
...
'latest_cell' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		$__finalCompiled .= '
		<div class="structItem-cell structItem-cell--latest ' . ($__vars['uix_originalPosterLastPosted'] ? 'uix_posterIsUser' : '') . '">
		';
	if ($__templater->filter($__vars['thread']['reply_count'], array(array('number', array()),), false) == 0) {
		$__finalCompiled .= '
			';
		$__vars['uix_originalPosterLastPosted'] = '1';
		$__finalCompiled .= '
		';
	}
	$__finalCompiled .= '
	...
	if ($__vars['thread']['discussion_type'] == 'redirect') {
		...
		$__finalCompiled .= '
				';
		if ((!$__vars['forum']) AND (!$__templater->func('is_addon_active', array('ThemeHouse/Filters', ), false))) {
			$__finalCompiled .= '
					<a class="uix_mobileNodeTitle" href="' . $__templater->func('link', array('forums', $__vars['thread']['Forum'], ), true) . '">' . $__templater->escape($__vars['thread']['Forum']['title']) . '</a>
				';
		}
		$__finalCompiled .= '
