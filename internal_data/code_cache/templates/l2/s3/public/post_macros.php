<?php
// FROM HASH: 3cf1d9a665044f3e3c8a2dec9cae2120
return array(
...
'user_cell' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		...
		'threadUserId' => $__vars['thread']['user_id'],
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
		...
		'itemprop' => 'datePublished',
			...
			';
	if ($__vars['post']['is_question_solution']) {
		$__finalCompiled .= '
				<li class="uix_solutionAttribution">				
					<span class="is-solution" data-xf-init="tooltip" title="' . 'Solution' . '">' . $__templater->fontAwesome('fa-check-circle', array(
		)) . ' ' . 'Solution' . '</span>
				</li>
			';
	}
	...
	if ($__templater->method($__vars['post'], 'isUnread', array())) {
		...
		$__finalCompiled .= '
				<li><span class="message-newIndicator">' . 'New replies' . '</span></li>
			';
	}
	$__finalCompiled .= '
			<li>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['post']['last_edit_date']) {
		...
		if ($__vars['post']['user_id'] == $__vars['post']['last_edit_user_id']) {
			$__finalCompiled .= '
				' . 'Chỉnh sửa lần cuối' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
				'itemprop' => 'dateModified',
			))) . '
				';
		} else {
			$__finalCompiled .= '
				' . 'Chỉnh sửa lần cuối bởi người điều hành' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
				'itemprop' => 'dateModified',
			))) . '
			';
		}
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__compilerTemp1 .= '
				...
				';
	if (($__templater->func('property', array('uix_voteActionButton', ), false) != 'never')) {
		$__compilerTemp1 .= '
		...
		if ($__templater->method($__vars['post'], 'isFirstPost', array())) {
			$__compilerTemp1 .= '
						';
			if ($__templater->method($__vars['post']['Thread'], 'isContentVotingSupported', array())) {
				$__compilerTemp1 .= '
...
';
	...
	<div class="message message--deleted message--post' . ($__templater->method($__vars['post'], 'isIgnored', array()) ? ' is-ignored' : '') . ($__templater->method($__vars['post'], 'isUnread', array()) ? ' is-unread' : '') . ' js-post js-inlineModContainer"
		...
		'log' => $__vars['post']['DeletionLog'],
						...
						<a href="' . $__templater->func('link', array('posts/show', $__vars['post'], ), true) . '" class="u-jsOnly" data-xf-click="inserter" data-replace="[data-content=post-' . $__templater->escape($__vars['post']['post_id']) . ']">' . 'Hiện' . $__vars['xf']['language']['ellipsis'] . '</a>
