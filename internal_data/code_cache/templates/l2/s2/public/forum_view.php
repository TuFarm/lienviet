<?php
// FROM HASH: a43f07c5a309af15ba22e85e37b532da
return array(
	...
	$__finalCompiled .= '
					...
					<div class="menu menu--wide" data-menu="menu" aria-hidden="true"
						...
						<div class="menu-content">
							<h4 class="menu-header">' . 'Chỉ hiện' . $__vars['xf']['language']['label_separator'] . '</h4>
...
'thread_list' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (!$__templater->test($__vars['stickyThreads'], 'empty', array()) OR !$__templater->test($__vars['threads'], 'empty', array())) {
		...
		if ($__vars['showDateLimitDisabler']) {
			$__finalCompiled .= '
								<div class="structItem js-emptyThreadList">
									<div class="structItem-cell">' . 'Không có chủ đề.' . '</div>
								</div>
								' . $__templater->callMacro(null, 'date_limit_disabler', array(
				'forum' => $__vars['forum'],
				'page' => $__vars['page'],
	...
	if ($__vars['filters']['last_days'] AND $__vars['dateLimits'][$__vars['filters']['last_days']]) {
		$__finalCompiled .= '
			<li><a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__templater->filter($__vars['filters'], array(array('replace', array('last_days', null, )),), false), ), true) . '"
				class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
				<span class="filterBar-filterToggle-label">' . 'Cập nhật lần cuối' . $__vars['xf']['language']['label_separator'] . '</span>
...
'order' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['filters']['order']) {
		...
		if ($__vars['filters']['direction'] == 'asc') {
			$__finalCompiled .= 'Dưới lên';
		} else {
			$__finalCompiled .= 'Trên xuống';
		}
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
	';
	$__vars['dateLimits'] = array('-1' => 'Bất cứ lúc nào', '7' => '' . '7' . ' ngày', '14' => '' . '14' . ' ngày', '30' => '' . '30' . ' ngày', '60' => '' . '2' . ' tháng', '90' => '' . '3' . ' tháng', '182' => '' . '6' . ' tháng', '365' => '1 năm', );
	$__finalCompiled .= '
...
';
	...
	';
		if ($__templater->isTraversable($__vars['nodeTree'])) {
			foreach ($__vars['nodeTree'] AS $__vars['id'] => $__vars['node']) {
				$__finalCompiled .= '
		';
				if ($__vars['nodeExtras'][$__vars['id']]['hasNew']) {
					$__finalCompiled .= '
			';
					$__vars['unreadNode'] = '1';
					$__finalCompiled .= '
	...
	<div class="block uix_nodeList block--category ' . ($__templater->func('is_toggled', array('thuixsf-' . $__vars['forum']['node_id'], ), false) ? 'category--collapsed' : '') . '">
		';
		if ($__templater->func('property', array('uix_categoryStripOutsideWrapper', ), false) AND $__templater->func('property', array('uix_collapseibleSubForums', ), false)) {
			$__finalCompiled .= '
			...
			$__finalCompiled .= '
					<a href="' . $__templater->func('link', array('categories', $__vars['node'], ), true) . '" class="uix_categoryTitle" data-xf-init="' . (($__vars['uix_categoryDescriptionDisplay'] == 'tooltip') ? 'element-tooltip' : '') . '" data-shortcut="node-description">Sub Forum</a>
					';
			if ($__vars['unreadNode']) {
				$__finalCompiled .= '<span class="uix_newIndicator">Unread</span>';
			}
			$__finalCompiled .= '
				</div>
				';
			if (($__templater->func('property', array('uix_categoryCollapse', ), false) AND $__templater->method($__vars['xf']['visitor'], 'hasPermission', array('th_uix', 'collapseCategories', )))) {
				$__finalCompiled .= '
					<a class="u-ripple categoryCollapse--trigger"
					   ...
					   data-xf-init="toggle-storage"
					   data-storage-type="cookie"
					   data-storage-key="thuixsf-' . $__templater->escape($__vars['forum']['node_id']) . '">
