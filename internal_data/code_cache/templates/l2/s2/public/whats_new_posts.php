		...
		if ($__vars['findNew']['filters']['unread']) {
			$__compilerTemp1 .= '
							<li><a href="' . $__templater->func('link', array($__vars['rerunRoute'], $__vars['rerunData'], $__vars['rerunQuery'] + array('remove' => 'unread', ), ), true) . '"
								...
								<span class="filterBar-filterToggle-label">' . 'Chỉ hiện' . $__vars['xf']['language']['label_separator'] . '</span>
								' . 'Chưa đọc' . '</a></li>
						';
		}
		$__compilerTemp1 .= '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	array(
		...
		'label' => 'Nội dung được theo dõi',
		'_type' => 'option',
	),
	array(
		'name' => 'participated',
		'selected' => $__vars['findNew']['filters']['participated'],
		'label' => 'Chủ đề mà bạn đã tham gia',
		'_type' => 'option',
	),
	array(
		'name' => 'started',
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
			...
			foreach ($__vars['threads'] AS $__vars['thread']) {
				...
				if ($__vars['xf']['visitor']['user_id'] AND ((!$__vars['activityBarShown']) AND ($__vars['thread']['last_post_date'] < $__vars['xf']['session']['previousActivity']))) {
					...
					<div class="structItem structItem--note">
						<div class="structItem-cell">' . 'Chủ đề dưới đây đã không được cập nhật kể từ lần truy cập trước của bạn.' . '</div>
					</div>
					';
	...
	';
		...
		if ($__vars['xf']['visitor']['user_id'] AND ($__vars['findNew']['filters']['unread'] AND ($__templater->func('count', array($__vars['findNew']['filters'], ), false) == 1))) {
			$__finalCompiled .= '
				' . 'Bạn không có bài viết chưa đọc. Thay vào đó, bạn có thể <a href="' . $__templater->func('link', array($__vars['latestRoute'], null, array('skip' => 1, ), ), true) . '" rel="nofollow">xem tất cả các bài đăng mới nhất</a>.' . '
			';
		} else {
			$__finalCompiled .= '
				' . 'Không tìm thấy.' . '
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Chủ đề mới');
