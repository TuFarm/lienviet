	...
	if ($__templater->func('count', array($__vars['users'], ), false)) {
		...
		if ($__templater->isTraversable($__vars['users'])) {
			foreach ($__vars['users'] AS $__vars['user']) {
				$__finalCompiled .= '
						<li data-name="' . $__templater->filter($__vars['user']['username'], array(array('for_attr', array()),), true) . '" title="' . $__templater->filter('Hoạt động cuối', array(array('for_attr', array()),), true) . ': ' . $__templater->func('date_time', array($__vars['user']['siropu_chat_last_activity'], ), true) . '">
							';
