...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__vars['tagIcon'] = $__templater->preEscaped('
		...
		'title' => $__templater->filter('Thẻ', array(array('for_attr', array()),), false),
	)) . '
	...
	if ($__vars['editLink']) {
		$__finalCompiled .= '
				<a href="' . $__templater->escape($__vars['editLink']) . '" class="u-concealed--icon"
					...
					data-xf-init="tooltip" title="' . $__templater->filter('Sửa thẻ', array(array('for_attr', array()),), true) . '">' . $__templater->escape($__vars['tagIcon']) . '</a>
			';
	} else {
		$__finalCompiled .= '
